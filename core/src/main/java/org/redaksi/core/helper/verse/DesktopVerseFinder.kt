...
object DesktopVerseFinder {
    val TAG: String = DesktopVerseFinder::class.java.simpleName
    ...
    fun findInText(input: CharSequence?, detectorListener: DetectorListener) {
        val match_1 = reg.matcher(input)

        while (match_1.find()) {
            // to solve the problem of "Dan" book
            val firstLetter = match_1.group(3)[0]
            if (firstLetter == 'd' /* "dan" instead of "Dan" */) {
                continue
            } else {
                val beginVerse = match_1.start(2)
                val endVerse = match_1.end(2)
                ...
                var newPattern = match_1.group(2).replaceFirst("\\.".toRegex(), " ")

...
class VerseFinder {
    ...
    private var reg: Pattern = Pattern.compile(
        "(\\b)((($bookNames)\\.?\\s+)(\\d+(?:(?:-|:|(?:;\\s*\\d+:\\s*)|,|\\.|\\d|dan|\\s)+\\d+)?)(?!\\s*(?:$nofollow)\\.?\\s))",
        Pattern.CASE_INSENSITIVE or Pattern.MULTILINE
    )

    fun findInText(input: CharSequence?): List<Triple<Int, Int, String>> {
        val match1: Matcher = reg.matcher(input)
        val result: MutableList<Triple<Int, Int, String>> = mutableListOf()
        while (match1.find()) {
            // to solve the problem of "Dan" book
            val firstLetter: Char = match1.group(3).get(0)
            if (firstLetter == 'd' /* "dan" instead of "Dan" */) {
                continue
            } else {
                val beginVerse: Int = match1.start(2)
                val endVerse: Int = match1.end(2)
                ...
                var newPattern: String = match1.group(2).replaceFirst("\\.", " ")
                // if there is word "dan" in pattern, "dan" will remove
                if (newPattern.contains("dan")) {
                    newPattern = if (Pattern.matches(".*dan\\s*\\d*:.*", newPattern)) {
                        newPattern.replaceFirst("dan".toRegex(), ";")
                    } else {
                        newPattern.replaceFirst("dan".toRegex(), ",")
                    }
                }
                ...
                newPattern = newPattern.replace(",\\s*;".toRegex(), ";")
                newPattern = newPattern.replace("\\s+".toRegex(), " ")
                newPattern = newPattern.replace("\n+".toRegex(), " ")
                result.add(Triple(beginVerse, endVerse, newPattern))
            }
        }
        return result
    }
    ...
    interface DetectorListener {
        fun onVerseDetected(start: Int, end: Int, verse: String?): Boolean
    }

    interface VerseClickListener {
        fun onVerseClicked(position: Int, s: CharSequence?, start: Int, end: Int, verse: String?)
    }
}

...
object DesktopVerseParser {
    ...
    var numberRangeSplitter: Pattern = Pattern.compile("\\s*(;|,|dan)\\s*" /* NOT case insensitive */)

    var numberStartEndSplitter: Pattern = Pattern.compile("\\s*--?\\s*")

    var chapterVerse: Pattern = Pattern.compile("(\\d+)\\s*[:.]\\s*(\\d+)")

    var numbersOnly: Pattern = Pattern.compile("[0-9]+")

    var orderedBooks: Array<String> = arrayOf(
    ...
    init {
        var i = 0
        val len = orderedBooks.size
        while (i < len) {
            for (bookName in orderedBooks[i].split("\\|".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()) {
                bookNameToId[bookName] = i
            }
            i++
        }
    }
    ...
    fun verseStringToAri(verse: String?): IntArrayList? {
        val m = reg.matcher(verse)

        if (!m.find()) {
            return null
        }

        val bookName = m.group(3).lowercase(Locale.getDefault())
        ...
        val numbers = m.group(4)
        val ranges = numberRangeSplitter.split(numbers)
        for (range in ranges) {
                ...
                if (cvStart != 0) {
                    ...
                    if (numbersOnly.matcher(startend_1_trim).matches()) { // check for cases like "2:3-17" (chapter 2 verse 3 to chapter 2 verse 17) or "14-17" (chapter 14 to chapter 17)
                        ...
                        cvEnd = if ((cvStart and 0xff) == 0) { // cvStart has no verse number, so this is for cases like "14-17" (chapter 14 to chapter 17)
                            ...
                            cvStart and 0xff00 or startend_1_number
                        }
                    } else {
